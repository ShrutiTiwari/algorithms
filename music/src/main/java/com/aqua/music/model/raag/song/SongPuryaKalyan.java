...
public class SongPuryaKalyan extends AbstractSong {
	...
	private final static MusicalPhrase MDMG_MRGG_NRND_NRGRS = new MusicalPhrase(4).n(M_, D, M_, G, G, R_, G, G, N1, R_, N1, D1)
			.couple(N1, R_).n(G, R_, S);
	...
	private final static MusicalPhrase MGRSS = new MusicalPhrase(4).couple(M_, G).n( R_, S).e(S, 4);
	private final static MusicalPhrase PMDPMG = new MusicalPhrase(4).couple(P, M_, D, P).n(M_, G);

	protected MusicalPhrase antara3rdLine() {
		return new MusicalPhrase(beatsPerDivision, 2).n(R_, G, R_, G).couple(G, M_).e(P, 2).e(M_, 2).n(D, N).couple(D, N).n(D)
				.couple(R3_, N).n(D, P);
	}

	protected MusicalPhrase antara4thLine() {
		return sthay2ndLine().add(sthayi3rdLine()).add(sthayi4thLine()).add(sthayi5thLine());
	}

	@Override
	protected Collection<MusicalPhrase> antaraExtraLines() {
		Collection<MusicalPhrase> result = new ArrayList<MusicalPhrase>();
		result.add(antara3rdLine());
		result.add(antara4thLine());
		return result;
	}

	@Override
	protected MusicalPhrase antaraFirstLine() {
		return new MusicalPhrase(beatsPerDivision).n(M_, D, M_, D).couple(D, N).n(S3, S3, S3).e(S3, 2).n(S3, S3, N, R3_).e(S3, 2);
	}

	@Override
	protected MusicalPhrase antaraSecondLine() {
		return new MusicalPhrase(beatsPerDivision, 2).n(N, N, R3_).couple(G3, R3_).n(G3, R3_).e(S3, 3).n(N, R3_, N).couple(D, N).n(D)
				.couple(R3_, N).n(D, P);
	}
	...
	protected MusicalPhrase sthayiFirstLineVariation() {
		return new MusicalPhrase(beatsPerDivision).add(GMRG).add(sthayi4thLine());
	}

	@Override
	protected MusicalPhrase sthayiSecondLine() {
		return sthay2ndLine().add(sthay2ndLine()).add(sthayi3rdLine()).add(sthayi3rdLine()).add(sthayi4thLine()).add(sthayi5thLine());
	}

	private MusicalPhrase sthay2ndLine() {
		return new MusicalPhrase(4).add(MDMG_MRGG_NRND_NRGRS);
	}

	private MusicalPhrase sthayi3rdLine() {
		return new MusicalPhrase().add(PMDPMG).add(MDNDP_M).add(GMRG);
	}

	private MusicalPhrase sthayi4thLine() {
		return new MusicalPhrase(beatsPerDivision).add(MDPMG).add(MDNDP_M).add(GMRG);
	}

	private MusicalPhrase sthayi5thLine() {
		return new MusicalPhrase().add(MDPMG).add(MGRSS);
	}
}
